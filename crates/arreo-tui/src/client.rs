...
use arreo_core::identity::{verifying_key_from_hex, DeviceCert, DeviceId, DeviceKey, VerifyingKey};
...
enum Link {
    ...
    Remote {
        channel: arreo_core::transport::SecureChannel,
        ...
        /// ask when it closed.
        session: Box<arreo_core::relay::session::RelaySession>,
    },
}
...
impl Client {
    ...
    #[must_use]
    pub fn closed(&self) -> Option<Arc<arreo_core::relay::session::Closed>> {
        match &self.link {
            Link::Local { .. } => None,
...
impl Link {
    ...
    async fn remote(remote: &RemoteTarget) -> Result<Self, ClientError> {
        ...
        for attempt in 1..=REMOTE_HANDSHAKE_ATTEMPTS {
            ...
            let handshake = arreo_core::transport::SecureChannel::connect(
                stream,
                &local,
                &ours,
                &remote.server_key,
