...
- [x] **The delta travels.** A version vector per synced file (machine → counter), exchanged
...
- [x] **Keep-both, never last-writer-wins.** Concurrent edits to one file leave the loser as
...
- [x] **The three merge hazards T-0075 measured, handled rather than discovered**: opencode
...
- [x] **A delta is verified before it is written**: the payload is scanned with the same
...
- [x] **`--slice mesh` extended** (not a second slice): two daemons on the existing relay
...
- [x] `cargo xtask e2e --slice compat` still green (the window is the referee for any protocol
