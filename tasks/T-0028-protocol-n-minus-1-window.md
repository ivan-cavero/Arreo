...
- [x] `negotiate(server, wants)` becomes a window: accept the highest version common to
...
- [x] Rules written in an ADR (number assigned at write time, avoiding sibling collisions)
...
- [x] Classification without a full decode: a `{op, v}` probe reads the map head, so an
...
- [x] Compat matrix in `crates/arreo-core/tests/compat.rs` over committed corpora (v0 frozen
...
- [x] Real-binary proof, new slice: `cargo xtask e2e --slice compat` in
...
- [x] Refusals are recorded, never partial: a refused connection writes an audit row and
      leaves no session, and the 1 MB budget plus T-0013's fuzz-total decode still hold.
- [x] Evidence under `.loop/evidence/T-0028/`: the rules table, both directions'
...
- **`rmp-serde` encodes internally-tagged enums as arrays, not maps.** The
  classifier was written for `{"op": ...}` map heads and classified nothing —
  the actual bytes are `["hello", 0, ...]`, tag first, fields as siblings. The
  map-head walk handles both shapes now, and the compat suite pins every v0 verb
  to its side so a future encoding change breaks a test instead of silently
  misclassifying. An unknown op defaults to *request* (refused, connection open)
  because a client that sent something unknown asked for work; the reverse
  default would discard a future `send` as if it were news.
- **`negotiate` must not cap at `VERSION`.** The first draft filtered offers to
  `<= VERSION`, which made a v1 server unable to agree on v1 with a v1 client —
  the one case the window most needs to get right. The window is `[server-1,
  server]`, full stop; a binary running as a version speaks it by definition.
  The test pins `negotiate(1, &[1]) == Ok(1)` so the cap cannot come back.

Also in the same pass: refused handshakes now write `auth.reject` rows (refused,
never partial — the row precedes the Error frame so a client that disconnects on
reading it cannot take the record with it), the 1 MB frame budget moved into
`frame_body_len` (refused before allocation, so no caller decides "too big" for
itself), and per-verb `check_version` stays as the second layer behind the
handshake window.
