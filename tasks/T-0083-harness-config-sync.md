...
- [x] **Per-file presets.** In a portable field, an env reference is not a secret —
...
- [x] **Per-file presets**, per `docs/harness-centralization.md`: SYNC files (opencode
...
- [x] **Keychain bridge.** The synced file carries `NAME`; the secret is injected at PTY spawn
...
- [x] **Conflict and history**: version vectors, keep-both with the loser as
...
- [x] **`xtask sync --check`** drives the whole story on two isolated roots standing in for two
      ...
      `.loop/evidence/T-0083/`.
- [x] The merge hazards T-0075 verified are handled, not discovered later: opencode's
...
The scanner half is `e5f355f`; the local half (presets, vectors, history, revert,
keychain bridge, the worked case) is `42d3c5b`. The security review of the engine
found eight issues in the hostile-payload path — three HIGH: a reference could
inject `mcp` syntax into the receiver's config, a forged vector bought a silent
overwrite, and an unquoted `${ARREO_ENV:NAME}` bypassed the by-name refusal — all
fixed with mutation-checked tests in `fa13ca9` (evidence:
`.loop/evidence/T-0083/scan-negative-controls.txt`).

**Two things this task did NOT do, both deliberately:** the delta exchange with a
real peer (the mesh transport, the conflict copies across two live machines, and
the JSONC/array merge hazards in their networked form) is **T-0086**; and the
daemon's PTY spawn does not yet *apply* the keychain plan, which is **T-0087** —
the mechanism is proven at a real pty in the slice, the wiring is not built.
