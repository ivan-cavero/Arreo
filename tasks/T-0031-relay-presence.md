...
- [ ] One rule, in code and as the only place it lives: heartbeat every **30 s**; `online`
      ...
      window.
- [ ] Boundaries are pinned with an injected clock (tests never sleep): 0 s / 89 s / 90 s / 91 s
...
- [ ] Lifecycle truth: connect writes `online`; heartbeats refresh `last_seen` every 30 s
...
- [ ] The 15-day case reads honestly — `offline (last seen 15 d ago)`, never an error — and that
      formatting is what `arreo machines` (T-0044) and remote attach (T-0032) render, asserted
      for 0 s / hour / day / month shapes.
- [ ] Read path is indexed and cheap: listing 10,000 devices is one query (`EXPLAIN QUERY PLAN`
      shows the index, no table scan) completing in < 50 ms on the dev box, asserted not assumed.
- [ ] Presence holds metadata only: the row is `(device_id, account_id, presence, last_seen)`, a
...
- [ ] Evidence under `.loop/evidence/T-0031/`: the boundary table, restart transcript, 10k-device
