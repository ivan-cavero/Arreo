...
- [x] Default unchanged: `q`/`Esc` quits the TUI, daemon + panes untouched (existing tui
      slice keeps passing unmodified).
- [x] Opt-in path sends the graceful drain-stop (T-0012 `server stop` semantics: flush
      rings + checkpoint, exit 0) — never `kill -9`, never unlink-while-serving.
- [x] Guards, all loud: refuses on `--machine`/remote targets (local socket only);
...
- [x] Config + flag: `tui.exit_kills_daemon = false` default; `--shutdown-on-exit` /
      `--no-shutdown-on-exit` override for the run; `--help` documents both.
- [x] Proven on a pty: (a) default quit leaves daemon serving, (b) flag quit with no
