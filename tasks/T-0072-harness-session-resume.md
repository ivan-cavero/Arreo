...
- [x] Per-pane record carries `harness + session-id` alongside program/args (schema
      migration, old rows restore as today — no orphan DBs).
- [x] Each adapter declares its resume strategy in data (`adapters/*.toml` + native
...
- [x] Boot restore uses the resume argv when present; resume failure falls back loudly
      to respawn+history (one bad record never blocks the rest, same contract as T-0018).
- [x] Proven live against real pi + opencode CLIs: spawn → produce session → kill -9
...
- [x] Safety: resume argv is constructed, never replayed keystrokes (T-0018's proven
