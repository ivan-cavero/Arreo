...
fn e2e(rest: &[String]) -> ExitCode {
    ...
    match slice {
        ...
        Some(other) => {
            eprintln!("xtask e2e: unknown slice {other:?} (have: chaos, api, compat, lifecycle, persistence, state, enforcement, tui, theme, relay, reattach, handoff-abort, mesh, update)");
            ExitCode::from(2)
        }
        None => stub("e2e", rest),
    }
